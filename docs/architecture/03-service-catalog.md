...
`loom-contracts`), the data it owns, and which of today's files it absorbs.
...
> call the service trait. SQL lives behind `loom-storage` repositories.

## 3.1 `loom-domain`
- **Responsibility:** shared value types and error kinds. Pure data, no I/O, no async, no
  dependency on any other loom crate.
...
- **Rule:** the only crate that writes SQL. Returns `loom-domain` types.
...
- **Implements:** `SettingsService`; uses `loom-crypto` + `SettingsRepo`.
